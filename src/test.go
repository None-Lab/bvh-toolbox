...
func (bt *BvhTree) GetJointIndex(name string) int {
	for i, joint := range bt.GetJoints(true) {
		if joint.Name() == name {
			return i
		}
	}
...
func (bt *BvhTree) GetHierarchyString() string {
	...
	s = bt.CloseScopes(s, 0) // Close all remaining scopes at the end
	return s
}
...
func WriteJointRotations(bvhTree *BvhTree, filePath string) bool {
	startTime := time.Now()
	...
	data := make([][]float64, len(bvhTree.GetJoints()))
	header := []string{"time"}

	for i, joint := range bvhTree.GetJoints() {
		...
		data[i] = append(data[i], bvhTree.FramesJointChannels(joint.Name(), rotationChannels, 0.0)...)
	}
...
func WriteJointPositions(bvhTree *BvhTree, filePath string, scale float64, endSites bool) bool {
	...
	getWorldPositions = func(joint *BvhNode) {
		...
		if joint.Value[0] == "End" {
			...
			offset = joint.JointOffset(joint.Name())
		}
		...
		for _, child := range joint.JointChildren(joint.Name()) {
			if endSites {
...
func WriteJointHierarchy(bvhTree *BvhTree, filePath string, scale float64) bool {
	...
	for _, joint := range bvhTree.GetJoints(true) {
		...
		offset := joint.JointOffset(jointName)
...
func Bvh2Csv(bvhPath string, dstDirpath string, scale float64, exportRotation bool, exportPosition bool, exportHierarchy bool, endSites bool) bool {
	...
	if exportPosition {
		posSuccess = WriteJointPositions(bvhTree, fmt.Sprintf("%s/%s_pos.csv", dstDirpath, strings.TrimSuffix(os.PathBase(bvhPath), ".bvh")), scale, endSites)
	}

	if exportRotation {
		rotSuccess = WriteJointRotations(bvhTree, fmt.Sprintf("%s/%s_rot.csv", dstDirpath, strings.TrimSuffix(os.PathBase(bvhPath), ".bvh")))
	}

	if exportHierarchy {
		hierarchySuccess = WriteJointHierarchy(bvhTree, fmt.Sprintf("%s/%s_hierarchy.csv", dstDirpath, strings.TrimSuffix(os.PathBase(bvhPath), ".bvh")), scale)
	}
...
func main() {
	// Example usage:
	bvhPath := "example_files/source/walk01.bvh"
	dstDirpath := "output"
	...
	if err := PlotCsv(fmt.Sprintf("%s/%s_pos.csv", dstDirpath, strings.TrimSuffix(os.PathBase(bvhPath), ".bvh")), 0, 1); err != nil {
