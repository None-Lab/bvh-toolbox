...
func WriteJointRotations(bvhTree *BvhTree, filePath string, endSites bool) bool {
	...
	for i, joint := range bvhTree.GetJoints(true) {
		...
		// Flatten the [][]float64 to []float64:
		flattenedData := make([]float64, 0)
		for _, frame := range bvhTree.FramesJointChannels(joint.Name(), rotationChannels, 0.0) {
			flattenedData = append(flattenedData, frame...)
		}

		data[i] = append(data[i], flattenedData...)
	}
...
func WriteJointPositions(bvhTree *BvhTree, filePath string, scale float64, endSites bool) bool {
	...
	getWorldPositions = func(joint *BvhNode) {
		...
		if joint.Value[0] == "End" {
			...
			// Correctly use bvhTree.JointOffset:
...
func Bvh2Csv(bvhPath string, dstDirpath string, scale float64, exportRotation bool, exportPosition bool, exportHierarchy bool, endSites bool) bool {
	...
	if exportPosition {
		posSuccess = WriteJointPositions(bvhTree, fmt.Sprintf("%s/%s_pos.csv", dstDirpath, path), scale, endSites)
	}

	if exportRotation {
		rotSuccess = WriteJointRotations(bvhTree, fmt.Sprintf("%s/%s_rot.csv", dstDirpath, path), endSites)
	}

	if exportHierarchy {
		hierarchySuccess = WriteJointHierarchy(bvhTree, fmt.Sprintf("%s/%s_hierarchy.csv", dstDirpath, path), scale)
	}
...
func main() {
	// Example usage:
	bvhPath := "test/ex/PT_S104_1_FM_M_009.bvh"
	dstDirpath := "test/csv"
	...
	if err := PlotCsv(fmt.Sprintf("%s/%s_pos.csv", dstDirpath, strings.TrimSuffix(path, ".bvh")), 0, 1); err != nil {
